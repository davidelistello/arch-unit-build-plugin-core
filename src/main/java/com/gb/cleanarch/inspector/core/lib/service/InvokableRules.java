...
class InvokableRules {
    ...
    private InvokableRules(String rulesClassName, List<String> ruleChecks, Log log) {
        ...
        rulesLocation = ReflectionUtils.loadClassWithContextClassLoader(rulesClassName);
    ...
    InvocationResult invokeOn(JavaClasses importedClasses) {
        ...
        Object instance = ReflectionUtils.newInstance(rulesLocation);
        ...
        for (Method method : archRuleMethods) {
            checkForFailure(() -> ReflectionUtils.invoke(method, instance, importedClasses))
                    .ifPresent(result::add);
        }
        for (Field field : archRuleFields) {
            ArchRule rule = ReflectionUtils.getValue(field, instance);
